...
public class CountryPickerDialog extends AppCompatDialog {
    ...
    public CountryPickerDialog(Context context, CountryPickerCallbacks callbacks, boolean showDialingCode, int raw, String selectedCountry) {
        super(context);
        ...
        if(selectedCountry != null && !selectedCountry.isEmpty())
        {
            ...
            {
                if (mList.get(x).getIsoCode().equals(selectedCountry))
                {
                    sCountry = mList.get(x);
                    sCountry.setSelectedIsoCode(true);
                    mList.remove(sCountry);
                    mList.add(0,sCountry);
                    break;
                }
            }
        }
